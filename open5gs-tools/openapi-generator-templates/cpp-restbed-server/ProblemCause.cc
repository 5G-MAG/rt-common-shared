...
const ProblemCause ProblemCause::INVALID_API(ProblemCause::registerCause(
        "INVALID_API", 400, "Unsupported API or API version", "The HTTP request contains an unsupported API name or API version in the URI."));
const ProblemCause ProblemCause::INVALID_MSG_FORMAT(ProblemCause::registerCause(
        "INVALID_MSG_FORMAT", 400, "Invalid Message Format", "The HTTP request has an invalid format."));
const ProblemCause ProblemCause::INVALID_QUERY_PARAM(ProblemCause::registerCause(
        "INVALID_QUERY_PARAM", 400, "Invalid Query Parameter", "The HTTP request contains an unsupported query parameter in the URI. "));
const ProblemCause ProblemCause::MANDATORY_QUERY_PARAM_INCORRECT(ProblemCause::registerCause(
        "MANDATORY_QUERY_PARAM_INCORRECT", 400, "Mandatory Query Parameter Incorrect", "A mandatory query parameter, or a conditional query parameter but mandatory required, for an HTTP method was received in the URI with semantically incorrect value."));
const ProblemCause ProblemCause::OPTIONAL_QUERY_PARAM_INCORRECT(ProblemCause::registerCause(
        "OPTIONAL_QUERY_PARAM_INCORRECT", 400, "Optional Query Parameter Incorrect", "An optional query parameter for an HTTP method was received in the URI with a semantically incorrect value that prevents successful processing of the service request."));
...
const ProblemCause ProblemCause::TOKEN_CCA_MISMATCH(ProblemCause::registerCause(
        "TOKEN_CCA_MISMATCH", 403, "Mismatch Between Token and CCA", "The request is rejected due to a mismatch between the subject claim in the access token and subject claim in the CCA."));
const ProblemCause ProblemCause::MODIFICATION_NOT_ALLOWED(ProblemCause::registerCause(
        "MODIFICATION_NOT_ALLOWED", 403, "Modification Not Allowed", "The request is rejected because the contained modification instructions attempt to modify IE which is not allowed to be modified."));
const ProblemCause ProblemCause::SUBSCRIPTION_NOT_FOUND(ProblemCause::registerCause(
        "SUBSCRIPTION_NOT_FOUND", 404, "Subscription Not Found", "The request for modification or deletion of subscription is rejected because the subscription is not found in the NF."));
const ProblemCause ProblemCause::RESOURCE_URI_STRUCTURE_NOT_FOUND(ProblemCause::registerCause(
        "RESOURCE_URI_STRUCTURE_NOT_FOUND", 404, "Resource URI Structure Not Found", "The request is rejected because a fixed part after the first variable part of an \"apiSpecificResourceUriPart\" (as defined in clause 4.4.1 of 3GPP TS 29.501) is not found in the NF.\n\nThis fixed part of the URI may represent a sub-resource collection (e.g. contexts, subscriptions, policies) or a custom operation."));
const ProblemCause ProblemCause::INCORRECT_LENGTH(ProblemCause::registerCause(
        "INCORRECT_LENGTH", 411, "Incorrect Content-Length", "The request is rejected due to incorrect value of a Content-length header field."));
const ProblemCause ProblemCause::NF_CONGESTION_RISK(ProblemCause::registerCause(
        "NF_CONGESTION_RISK", 429, "NF Congestion Risk", "The request is rejected due to excessive traffic which, if continued over time, may lead to (or may increase) an overload situation of the NF instance."));
const ProblemCause ProblemCause::NF_SERVICE_CONGESTION_RISK(ProblemCause::registerCause(
        "NF_SERVICE_CONGESTION_RISK", 429, "NF Service Congestion Risk", "The request is rejected due to excessive traffic which, if continued over time, may lead to (or may increase) an overload situation of the NF service instance."));
const ProblemCause ProblemCause::INSUFFICIENT_RESOURCES(ProblemCause::registerCause(
        "INSUFFICIENT_RESOURCES", 500, "Insufficient Resources", "The request is rejected due to insufficient resources."));
const ProblemCause ProblemCause::UNSPECIFIED_NF_FAILURE(ProblemCause::registerCause(
        "UNSPECIFIED_NF_FAILURE", 500, "Unspecified NF Failure", "The request is rejected due to unspecified reason at the NF."));
const ProblemCause ProblemCause::SYSTEM_FAILURE(ProblemCause::registerCause(
        "SYSTEM_FAILURE", 500, "System Failure", "The request is rejected due to generic error condition in the NF."));
const ProblemCause ProblemCause::NF_FAILOVER(ProblemCause::registerCause(
        "NF_FAILOVER", 500, "NF Unavailable", "The request is rejected due to the unavailability of the NF, and the requester may trigger an immediate re-selection of an alternative NF based on this information."));
const ProblemCause ProblemCause::NF_SERVICE_FAILOVER(ProblemCause::registerCause(
        "NF_SERVICE_FAILOVER", 500, "NF Service Unavailable", "The request is rejected due to the unavailability of the NF service, and the requester may trigger an immediate re-selection of an alternative NF service based on this information."));
const ProblemCause ProblemCause::INBOUND_SERVER_ERROR(ProblemCause::registerCause(
        "INBOUND_SERVER_ERROR", 502, "External Server Error", "The request is rejected due to the receipt of an 5xx error from an inbound server that the NF Service Producer accessed while attempting to fulfil the request (see clause 6.4.2.1 of 3GPP TS 29.500)."));
const ProblemCause ProblemCause::NF_CONGESTION(ProblemCause::registerCause(
        "NF_CONGESTION", 503, "NF Congestion", "The NF instance experiences congestion and performs overload control, which does not allow the request to be processed."));
const ProblemCause ProblemCause::NF_SERVICE_CONGESTION(ProblemCause::registerCause(
        "NF_SERVICE_CONGESTION", 503, "NF Service Congestion", "The NF service instance experiences congestion and performs overload control, which does not allow the request to be processed."));
const ProblemCause ProblemCause::TARGET_NF_NOT_REACHABLE(ProblemCause::registerCause(
        "TARGET_NF_NOT_REACHABLE", 504, "Target NF Not Reachable", "The request is not served as the target NF is not reachable."));
const ProblemCause ProblemCause::TIMED_OUT_REQUEST(ProblemCause::registerCause(
        "TIMED_OUT_REQUEST", 504, "Request Timed Out", "The request is rejected due a request that has timed out at the HTTP client (see clause 6.11.2 of 3GPP TS 29.500)."));
